 ...
 *  用于配置Spark application, 所使用的是K,V的值对，用来配置spark的运行时参数
 ...
 * `new SparkConf().setMaster("local").setAppName("My app")`.常用的set写法：
 *
 * Note that once a SparkConf object is passed to Spark, it is cloned and can no longer be modified
 * by the user. Spark does not support modifying the configuration at runtime.
 *
 * @param loadDefaults whether to also load values from Java system properties
 *                 参数：是否从Java System环境变量中load参数
 */
class SparkConf(loadDefaults: Boolean) extends Cloneable with Logging {
  ...
  /** 默认是加载系统的classpath环境变量*/
  def this() = this(true)

  private[spark] val settings = new HashMap[String, String]()       //用HashMap放参

  if (loadDefaults) {  //去获取当前用户系统的环境变量参数，只加载spark开头的参数值
