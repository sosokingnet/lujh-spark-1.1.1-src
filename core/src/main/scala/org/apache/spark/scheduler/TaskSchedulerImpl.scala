 ...
 * 基本逻辑：决定job的执行策略，org.apache.spark.scheduler.TaskSchedulerImpl通过一个SchedulerBackend管理了所有的cluster的调度；它主要实现了通用的逻辑。对于系统刚启动时，需要理解两个接口，一个是initialize，一个是start
...
//backend这里为null，交由后面根据不同的参数模式，分配不同的SchedulerBackend实现子类，从而实现不同的scheduler调度模式
  ...
  // #lujh,【重要】：目前实现的调度方式——现进先出FIFO，配置的变量：spark.scheduler.mode，有配置就是配置的，没有的话默认是FIFO
  ...
  def initialize(backend: SchedulerBackend) {
    ...
    schedulableBuilder = {
      schedulingMode match {             //初始化，根据参数，创建FIFO或者FAIR的调度队列，目前只支持这两种调度模式
  ...
  //#lujh:【重要】整个SC的启动，调度器的START，会走到这里
  override def start() {
    backend.start()
//对于非本地模式，并且设置了spark.speculation为true
    if (!isLocal && conf.getBoolean("spark.speculation", false)) {
      logInfo("Starting speculative execution thread")
      import sc.env.actorSystem.dispatcher  //分发？下面涉及一些系统配置的参数，注意
