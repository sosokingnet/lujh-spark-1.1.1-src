...
//相关信息：spark.scheduler.minRegisteredResourcesRatio
//The minimum ratio of registered resources   最小的注册资源比率，如果低于这个数值，就不执行，直到有足够的资源，资源用executor来表示
// (registered resources / total expected resources)
// (resources are executors in yarn mode, CPU cores in standalone mode) to wait for before scheduling begins.
// Specified as a double between 0 and 1. Regardless of whether the minimum ratio of resources has been reached,
// the maximum amount of time it will wait before scheduling begins is controlled by config spark.scheduler.maxRegisteredResourcesWaitingTime .
  override def isReady(): Boolean = {
    if (sufficientResourcesRegistered) {
      logInfo("SchedulerBackend is ready for scheduling beginning after " +         //15/01/11 22:58:17 INFO YarnClientSchedulerBackend: SchedulerBackend is ready for scheduling beginning after reached minRegisteredResourcesRatio: 0.8
