...
/*
 * 执行$SPARK_HOME/sbin/start-master.sh时候，用到的就是当前这个类了
 * local cluster模式是一种伪cluster模式，在单机环境下模拟standalone的集群，启动顺序分别如下
 * 启动master
 * 启动worker
 * 启动spark-shell
 * master主要是运行类 org.apache.spark.deploy.master.Master，在8080端口启动监听
 * masterUrl变量应该记录监听的地址+端口，例如localhost:8080,MasterWebUI的地址
 */
